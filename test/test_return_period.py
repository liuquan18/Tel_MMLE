...
plev = [100000, 50000]
mode = ["NAO", "EA"]
values = np.array([[[np.arange(-5, 6, 1)] * 10] * 2] * 2).reshape(
    11, 10, 2, 2
)  # time,  ens, plev,mode

ex = xr.DataArray(
    values,
    dims=["time", "ens", "plev", "mode"],
    coords={"time": time, "ens": ens, "plev": plev, "mode": mode},
)
ex.name = "pc"
# %%
pos, mpos, neg, mneg = EVT.mode_return_period(
    ex, mode="NAO", periods=[slice("2010", "2015")], plev=50000
)
# %%
