...
#%%
TAllens = TAllens.sel(time = slice('1850','2015'))
#%%
TAllens = TAllens.sortby('time')
# %%
Allens_mean = glmean(TAllens.TMP)
#%%
# ERA5_mean = ERA5_mean.sel(time = slice('1979','2022'))
# Allens_mean = Allens_mean.sel(time = slice('1979','2022'))

# %%
Allens_mean = anomaly(Allens_mean)
...
# %%
plot_month_series(ERA5_mean=None, 
                  Allens_mean=Allens_mean, 
                  ens_mean=ens_mean, 
                  ens_mean_rm=ens_mean_rm, 
                  ens_mean_fit=ens_mean_fit,
                  period = None)

# %%
plot_series(    ERA5_mean=None, 
                  Allens_mean=Allens_mean, 
                  ens_mean=ens_mean, 
                  ens_mean_rm=ens_mean_rm, 
                  ens_mean_fit=ens_mean_fit,
...
ZERA5 = ZERA5/9.81
#%%
Zallens = read_data("CR20_allens",var = 'zg')
# %%
ZERA5_s = glmean(ZERA5.HGT.sel(lat = slice(60,35), lon = slice(-30,0)))
...
#%%
Zallens_s = glmean(Zallens.HGT.sel(lat = slice(60,35), lon = slice(-30,0)))
# %%

Zallens_n = glmean(Zallens.HGT.sel(lat = slice(75,60), lon = slice(-70,-50)))
# %%
...
# %%
plot_month_series(Zallens_s, Zens_mean_s, Zens_mean_s_rm, Zens_mean_s_fit, month = 7)

# %%
plot_series(ERA5_mean = None, 
            Allens_mean= Zallens_s, 
            ens_mean=Zens_mean_s, 
            ens_mean_rm=Zens_mean_s_rm, 
            ens_mean_fit=Zens_mean_s_fit,
            period=None)
# %%
def plot_std_ens(ens_data):
    fig, ax = plt.subplots(figsize=(10, 5))
    ens_data.std(dim = 'ens').plot(ax = ax)
#%%
def _iv(xarr):
    iv = xarr.std()
    return iv
