...
fig, ax = plt.subplots(1, 2, figsize=(12, 6))
# Plotting percentage of values <= 1.5 and -1.5
bars_1_5 = ax[0].bar(models, [percentage_1_5[model] for model in models], label="<= 1.5")
bars_minus_1_5 = ax[0].bar(models, [percentage_minus_1_5[model] for model in models], label="<= -1.5", alpha=0.7)
ax[0].set_xlabel("Models")
...
# Plotting 90th and 10th quantiles
bars_90 = ax[1].bar(models, [quantile_90[model] for model in models], label="90th quantile")
bars_10 = ax[1].bar(models, [quantile_10[model] for model in models], label="10th quantile", alpha=0.7)
ax[1].set_xlabel("Models")
...
for bar in bars_10:
    yval = bar.get_height()
    ax[1].text(bar.get_x() + bar.get_width()/2, yval - 0.1, f'{yval:.1f}', ha='center', va='bottom')

# Plotting 93rd and 7th quantiles as lines
ax[1].plot(models, [quantile_93[model] for model in models], label="93rd quantile", color='red', marker='o')
ax[1].plot(models, [quantile_7[model] for model in models], label="7th quantile", color='blue', marker='o')
ax[1].legend(ncol = 2)
...
plt.tight_layout()
plt.savefig("/work/mh0033/m300883/Tel_MMLE/docs/source/plots/paper_supplymentary/percentage_std.pdf")
