...
# %%
def read_data( ens_size,model = 'MPI_GE_onepct',plev = 50000):
    eof_dir = f"/work/mh0033/m300883/Tel_MMLE/data/{model}_random/EOF_result/plev_{plev}_decade_temporal_ens{str(ens_size)}_eof_result.nc"
...
def process_ens_size(ens_size):
    plev = 30000
    eof_result, tsurf = read_data(ens_size,plev = plev)
    ds = extreme_counts(eof_result, tsurf)
    ds.to_netcdf(f"/work/mh0033/m300883/Tel_MMLE/data/MPI_GE_onepct_random/extreme_count/plev_{plev}_extreme_counts_tsurf_{str(ens_size)}.nc")
