...
def remove_decade_running_mean(month_nao,rolling_years = [10,20,30]):
    ...
    for year in rolling_years:
        rolling_mean = rolling_mean.rolling(time=year, center=True).mean() # do rolling on the rolling_mean before. 
        rolling_mean = rolling_mean.interpolate_na(dim='time', method='slinear', fill_value="extrapolate")
...
def plot_era_nao_index(nao, NEW_nao, ax):
    ...
    # pos
    ax.text(0.20, 0.99, f"1941-1981: {first_pos_org}", transform=ax.transAxes, fontsize=7,
        color = 'gray',alpha = 0.8, verticalalignment='top')
    ax.text(0.55, 0.99, f"1982-2022: {last_pos_org}", transform=ax.transAxes, fontsize=7,
        color = 'gray',alpha = 0.8, verticalalignment='top')
    
    ax.text(0.20, 0.93, f"1941-1981: {first_pos_new}", transform=ax.transAxes, fontsize=7,
        color = 'black',alpha = 0.8, verticalalignment='top')
    ax.text(0.55, 0.93, f"1982-2022: {last_pos_new}", transform=ax.transAxes, fontsize=7,
        color = 'black',alpha = 0.8, verticalalignment='top')

    # neg
    ax.text(0.20, 0.15, f"1941-1981: {first_neg_org}", transform=ax.transAxes, fontsize=7,
        color = 'gray',alpha = 0.8, verticalalignment='top')
    ax.text(0.55, 0.15, f"1982-2022: {last_neg_org}", transform=ax.transAxes, fontsize=7,
        color = 'gray',alpha = 0.8, verticalalignment='top')
    
    ax.text(0.20, 0.10, f"1941-1981: {first_neg_new}", transform=ax.transAxes, fontsize=7,
        color = 'black',alpha = 0.8, verticalalignment='top')
    ax.text(0.55, 0.10, f"1982-2022: {last_neg_new}", transform=ax.transAxes, fontsize=7,
        color = 'black',alpha = 0.8, verticalalignment='top')
