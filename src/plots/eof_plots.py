...
def exp_time_height(exp_plot):
    ...
    naoexp = exp_plot.sel(plev=slice(200, 1000), mode="NAO").plot(
        x="time", y="plev", ax=axes[0], levels=np.arange(30, 46, 2), extend="both"
    )
    eaexp = exp_plot.sel(plev=slice(200, 1000), mode="EA").plot(
        x="time",
        y="plev",
        ax=axes[1],
...
def visu_eofspa(eofs, plev=[50000, 85000], levels=np.arange(-1, 1.1, 0.2)):
    ...
    for i, row in enumerate(axes):  # plev
        for j, col in enumerate(row):  # mode
            data = eofs.sel(plev=plev[i], mode=mode[j]).values
            im = col.contourf(
                eofs.lon,
                eofs.lat,
                data,
...
def visu_eofspa_all(eofs, mode="EA", levels=np.arange(-1, 1.1, 0.2)):
    ...
    for i, ax in enumerate(axes.flat):
        axbuild(ax)
        if i < len(eofs.plev):
            data = eofs.sel(mode=mode).isel(plev=i).values
            im = ax.contourf(
                eofs.lon,
                eofs.lat,
                data,
            ...
            ax.set_title("eof {}".format(eofs.isel(plev=i).plev.values))
    cbar_ax = fig.add_axes([0.85, 0.2, 0.03, 0.6])
    fig.colorbar(im, cax=cbar_ax, label="eofs")
...
def visu_spatial_type(eofs, plev, mode="EA"):
    ...
    """
    all_eof, first_eof, last_eof = [eof.sel(plev=plev) for eof in eofs]
    eof = xr.concat(
        [first_eof, all_eof, last_eof], dim="type", coords="minimal", compat="override"
    )
    eof["type"] = ["first", "all", "last"]

    EOFmaps = eof.sel(mode=mode).plot.contourf(
        "lon",
        "lat",
        col="type",
...
def visu_composite_spa(composite, plev=50000, levels=np.arange(-2, 2.1, 0.4)):
    ...
    for i, row in enumerate(axes):  # extr_type
        for j, col in enumerate(row):  # mode
            data = composite.sel(plev=plev, extr_type=extr_type[i], mode=mode[j]).values
            im = col.contourf(
                composite.lon,
                composite.lat,
                data,
...
def tenyr_hist(data, hlayer=50000, bins=50):
    ...
    line = ax.plot(
        np.arange(-3, 4, 1), np.arange(-3, 4, 1), linestyle="dotted", color="k"
    )
    ...
    for i, ax in enumerate(axes):
        if hlayer == "all":
            ...
            first_data, last_data = (
                first.loc[hlayer, modes[i], :],
                last.loc[hlayer, modes[i], :],
            )

        scaf = sns.scatterplot(
            data=first_data, x="pc_all", y="pc_first", ax=ax, label="first"
        )
        scar = sns.scatterplot(
            data=last_data,
            x="pc_all",
            y="pc_last",
            ax=ax,
        ...
        line = ax.plot(
            np.arange(-5, 5, 1), np.arange(-5, 5, 1), linestyle="dotted", color="k"
        )
        ...
        for i, period in enumerate(args):
            if hlayer == "all":
                firstPattern, lastPattern = [
                    period_ten.loc[:, mode, :, :] for period_ten in period
                ]
            else:
                firstPattern, lastPattern = [
                    period_ten.loc[hlayer, mode, :, :] for period_ten in period
                ]

            scatterfirst = sns.scatterplot(
                data=firstPattern,
                x="pc_all",
                y="pc_first",
                ax=row[i],
...
def vertical_profile(extreme_counts, mode="NAO"):
    ...
    type_legend = axes[-1].legend(
        custom_lines,
        ["first", "all", "last", "dynamic", "", "pos", "neg"],
        loc="lower right",
        fontsize=6,
    )
    ...
    for i, row in enumerate(axes):  # modes
        for j, ax in enumerate(row):  # ind
            for l, p in enumerate(patterns):
                data_mode = (
                    data[j]
                    .xs((p, modes[i]), level=["diff", "mode"], axis=1)
                    .sort_index()
                )
...
def scatter_pattern_counts(ind_pattern, dep_pattern, mode, fit_reg=True):
    ...
    **Return**
    ...
    ind = ind_pattern.xs((slice(70000, 100000), mode), level=("plev", "mode"))

    dep = dep_pattern.xs((slice(70000, 100000), mode), level=("plev", "mode"))
...
def vertical_profile_slides(extreme_counts, mode="NAO"):
    ...
    data_single = pd.concat(
        [first, last, dynamic], keys=["first", "last", "dynamic"], axis=1
    )
